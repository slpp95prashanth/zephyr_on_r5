...
void ti_dmtimer_isr(void *data)
{
	...
	sys_write32((1 << TI_DM_TIMER_IRQENABLE_SET_MAT_EN_FLAG_SHIFT) |
		(1 << TI_DM_TIMER_IRQSTATUS_TCAR_IT_FLAG_SHIFT),
	...
	DEBUG(".\nMAT = 0x%08x TCRR = 0x%08x\n", sys_read32(TI_DM_TIMER_TMAR), curr_cycle);
	DEBUG("ti_dmtimer_isr: curr_cycle = 0x%08x last_cycle = 0x%08x delta_cycles = 0x%08x delta_ticks = %u\n",
		curr_cycle, last_cycle, delta_cycles, delta_ticks);
	DEBUG(".\n");
}
...
void sys_clock_set_timeout(int32_t ticks, bool idle)
{
	...
	uint32_t adj = (curr_cycle - last_cycle) + (CYC_PER_TICK - 1);
	...
	DEBUG(",\n");
	DEBUG("timeout: MAT = 0x%08x TCRR = 0x%08x idle = %d\n", sys_read32(TI_DM_TIMER_TMAR), curr_cycle, idle);
	DEBUG("timeout: ticks = %d next_cycle = 0x%08x last_cycle = 0x%08x last_elapsed = 0x%08x CYC_PER_TICK = 0x%08x\n,\n",
		ticks, next_cycle, last_cycle, last_elapsed, CYC_PER_TICK);
#endif
}
